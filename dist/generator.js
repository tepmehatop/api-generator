...
class CodeGenerator {
     ...
     */
    generateApiReadme(tag, operations) {
        const lines = [];
        const displayName = tag.charAt(0).toUpperCase() + tag.slice(1);
        ...
        lines.push(`import { ${operations.slice(0, 3).map(op => (0, string_helpers_1.toCamelCase)(op.operationId)).join(', ')} } from '@your-company/api-codegen/${tag}';`);
        lines.push('');
        lines.push('// Пример:');
        ...
        if (this.config.transliterateRussian && /[а-яА-Я]/.test(tag)) {
            readmeFileName = (0, transliterate_1.transliterate)(tag);
        }
