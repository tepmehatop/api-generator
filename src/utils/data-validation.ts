...
export interface ValidationConfig {
  ...
  // НОВОЕ v14.5.4: Количество попыток валидации для POST/PUT/PATCH запросов
  /**
...
export async function validateRequest(
  ...
  let lastResult: { status: number; data: any; error?: any } | null = null;
...
function extract422DetailMessage(responseData: any): string {
  ...
  if (responseData.errors && Array.isArray(responseData.errors)) {
    if (responseData.errors.length === 0) return '';
