 ...
 */
function buildEndpointMethodsMap(methods) {
    const map = new Map();
 ...
 */
function getAllowedMethodsForEndpoint(endpointPath, endpointMethodsMap) {
...
async function generateNegativeTests(config) {
    const fullConfig = {
        ...
        exclude405Methods: [], // НОВОЕ v14.1: Глобально исключенные методы
 ...
 */
async function generateNegativeTestForMethod(method, config, endpointMethodsMap) {
    const lines = [];
    // Импорты
    ...
    if (config.generate405Tests) {
        ...
        const methodsToTest = ['GET', 'POST', 'PUT', 'DELETE'].filter(m => {
            if (m === method.httpMethod.toUpperCase())
                return false; // Текущий метод
            if (allowedMethods.includes(m))
                return false; // Разрешённый для endpoint
            if (globallyExcluded.includes(m))
                return false; // Глобально исключённый
            return true;
        });
 ...
 */
function generateTestDataSectionWithDb(schema, testDataFilePath) {
    const lines = [];
 ...
 */
function generatePairwiseTestDataSectionWithDb(schema, testDataFilePath) {
    const lines = [];
