...
class HappyPathTestGenerator {
    ...
    async appendTestsToFile(filePath, endpoint, method, requests) {
        ...
        const newTests = requests.map((req, index) => this.generateSingleTest(endpoint, method, req, index + 1)).join('\n\n');
    ...
    generateTestFile(endpoint, method, requests) {
        const tests = requests.map((req, index) => this.generateSingleTest(endpoint, method, req, index + 1)).join('\n\n');
...
`;
     ...
     */
    generateSingleTest(endpoint, method, request, testNumber) {
...
//# sourceMappingURL=happy-path-generator-old-v94.js.map