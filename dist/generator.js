...
class CodeGenerator {
    ...
    generate() {
        ...
        // Генерируем ОДИН общий README для всего сервиса со всеми тегами
        files.push(this.generateCombinedApiReadme(operationsByTag));
    ...
    generateIndexFile(tags) {
        const lines = [];
        lines.push('/**');
        lines.push(' * Barrel export для всех API методов');
        lines.push(' * Типы импортируйте из конкретных файлов для избежания конфликтов');
        lines.push(' * ');
        lines.push(' * @example');
        lines.push(' * // Методы (автоимпорт работает)');
        lines.push(' * import { createOrder, getOrderById } from "@company/api-codegen/orders"');
        lines.push(' * ');
        lines.push(' * // Типы (импортируйте из файлов)');
        lines.push(' * import type { CreateOrderRequest } from "@company/api-codegen/orders/orders.api"');
        lines.push(' */\n');
        // Экспорт базовых типов (они безопасны)
        if (this.baseSchemas.size > 0) {
            lines.push("// Базовые типы");
            lines.push("export * from './base.types';");
            lines.push('');
        }
        // Для каждого тега экспортируем ТОЛЬКО функции
        for (const tag of tags) {
            const filename = this.getTagFilename(tag).replace('.ts', '');
            const displayName = tag.charAt(0).toUpperCase() + tag.slice(1);
            lines.push(`// ${displayName} API - только методы`);
            lines.push(`export { `);
            // Находим все операции для этого тега
            const operations = this.spec.paths.filter(p => p.tags.includes(tag));
            const functionNames = [];
            for (const op of operations) {
                const funcName = (0, string_helpers_1.toCamelCase)(op.operationId);
        ...
        lines.push('// Для типов импортируйте напрямую из файлов:');
        for (const tag of tags) {
            const filename = this.getTagFilename(tag).replace('.ts', '');
            lines.push(`// import type { YourType } from './${filename}';`);
        }
        lines.push('');
        return {
            filename: 'index.ts',
     ...
     */
    generateCombinedApiReadme(operationsByTag) {
        const lines = [];
        // Заголовок
        lines.push(`# API Documentation`);
        lines.push('');
        lines.push(`Автоматически сгенерированная документация API.`);
        lines.push('');
        lines.push('---');
        lines.push('');
        // Оглавление
        ...
        for (const [tag, operations] of operationsByTag) {
            ...
            const methodNames = operations.slice(0, 3).map(op => (0, string_helpers_1.toCamelCase)(op.operationId));
            lines.push(`import { ${methodNames.join(', ')} } from '@your-company/api-codegen/${tag}';`);
            lines.push('');
            if (operations.length > 0) {
                const firstOp = operations[0];
                const funcName = (0, string_helpers_1.toCamelCase)(firstOp.operationId);
                lines.push('// Пример:');
            ...
            if (allTypes.size > 0) {
                ...
                const typesList = Array.from(allTypes).slice(0, 10);
