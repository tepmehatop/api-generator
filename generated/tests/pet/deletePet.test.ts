...
test.describe(`API тесты для эндпоинта ${httpMethod} >> ${endpoint}`, async () => {
  ...
  test(`${httpMethod} без TOKEN (${unauthorized}) @api`, async ({ page }, testInfo) => {
    await axios.delete(process.env.StandURL + endpoint).catch(async function(error) {
  ...
  test(`${httpMethod} с несуществующим ID (${notFound}) @api`, async ({ page }, testInfo) => {
    ...
    await axios.delete(process.env.StandURL + endpoint404, configApiHeaderAdmin).catch(async function(error) {
  ...
  test(`${httpMethod} успешный запрос (${success}) @api @positive`, async ({ page }, testInfo) => {
    const response = await axios.delete(process.env.StandURL + endpoint, configApiHeaderAdmin);
  ...
  // DTO для данного метода не найдено
  // Создаем базовые pairwise тесты с различными наборами данных

  test(`${httpMethod} pairwise вариант 1 (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const testData = {}; // TODO: заполнить вариант 1 данных
    const response = await axios.delete(process.env.StandURL + endpoint, testData, configApiHeaderAdmin);

    await expect(response.status).toBe(success);
    await expect(response.data).toBeDefined();
  });

  test(`${httpMethod} pairwise вариант 2 (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const testData = {}; // TODO: заполнить вариант 2 данных
    const response = await axios.delete(process.env.StandURL + endpoint, testData, configApiHeaderAdmin);
  ...
  test(`${httpMethod} pairwise вариант 3 (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const testData = {}; // TODO: заполнить вариант 3 данных
    const response = await axios.delete(process.env.StandURL + endpoint, testData, configApiHeaderAdmin);
