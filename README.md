...
1. **Ключевые поля** - объекты сортируются по первому найденному полю из списка:
   - `id`, `uuid`, `code`, `key`, `name`, `title`

2. **Хеш объекта** - если ключевого поля нет, создаётся хеш из всех значений

3. **Рекурсивная обработка** - вложенные массивы также сортируются
