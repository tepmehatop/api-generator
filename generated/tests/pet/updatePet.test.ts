...
import { getMessageFromResponse, getMessageFromError } from '../../../helpers/apiTestHelper';
...
test.describe(`API тесты для эндпоинта ${httpMethod} >> ${endpoint}`, async () => {
  ...
  test(`${httpMethod} без TOKEN (${unauthorized}) @api`, async ({ page }, testInfo) => {
    await axios.put(process.env.StandURL + endpoint, { /* TODO: заполнить тестовыми данными */ }).catch(async function(error) {
  ...
  test(`${httpMethod} с неверным Content-Type (${unsupportedMediaType}) @api`, async ({ page }, testInfo) => {
    ...
    await axios.put(process.env.StandURL + endpoint, { /* TODO: заполнить тестовыми данными */ }, wrongHeaders).catch(async function(error) {
  ...
  test(`${httpMethod} с обязательными полями (${success}) @api @positive`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, requiredFieldsOnly, configApiHeaderAdmin);
  ...
  test(`${httpMethod} со всеми полями (${success}) @api @positive`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, allFieldsFilled, configApiHeaderAdmin);
  ...
  test(`${httpMethod} pairwise комбинация 1 (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, pairwiseCombo1, configApiHeaderAdmin);
  ...
  test(`${httpMethod} pairwise комбинация 2 (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, pairwiseCombo2, configApiHeaderAdmin);
  ...
  test(`${httpMethod} pairwise комбинация 3 (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, pairwiseCombo3, configApiHeaderAdmin);
  ...
  test(`${httpMethod} pairwise комбинация 4 (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, pairwiseCombo4, configApiHeaderAdmin);
  ...
  test(`${httpMethod} с status='available' (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, pairwiseEnum_status_1, configApiHeaderAdmin);
  ...
  test(`${httpMethod} с status='pending' (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, pairwiseEnum_status_2, configApiHeaderAdmin);
  ...
  test(`${httpMethod} с status='sold' (${success}) @api @pairwise`, async ({ page }, testInfo) => {
    const response = await axios.put(process.env.StandURL + endpoint, pairwiseEnum_status_3, configApiHeaderAdmin);
